...
export class DatabaseStorage implements IStorage {
  ...
  async getAdminDashboardStats(): Promise<any> {
    try {
      ...
      // Active users (online or logged in within last 24 hours)
      ...
      // Verified users
      const verifiedUsersResult = await db.select({ count: sql<number>`count(*)` })
        .from(users)
        .where(eq(users.isVerified, true));
      const verifiedUsers = Number(verifiedUsersResult[0]?.count || 0);
      console.log(`✅ Verified users: ${verifiedUsers}`);

      // Total stores/vendors
      const totalStoresResult = await db.select({ count: sql<number>`count(*)` }).from(vendors);
      const totalStores = Number(totalStoresResult[0]?.count || 0);
      ...
      const pendingVerificationsResult = await db.select({ count: sql<number>`count(*)` })
